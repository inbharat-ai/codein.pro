...
export async function getIgnoreContext(
  ...
  // These are done separately so that .codeignore/.continueignore can override .gitignore
  const gitIgnoreFile = dirFiles.find((name) => name === ".gitignore");
  const codeinIgnoreFile = dirFiles.find(
    (name) => name === ".codeignore" || name === ".continueignore",
  );
  ...
  const getCodeinIgnorePatterns = async () => {
    if (codeinIgnoreFile) {
      const contents = await ide.readFile(`${currentDir}/${codeinIgnoreFile}`);
      return gitIgArrayFromFile(contents);
    }
    return [];
  };
  ...
  const ignoreContext = ignore()
    ...
    .add(defaultAndGlobalIgnores) // default file/folder ignores followed by global .codeignore - this is combined for speed
    .add(ignoreArrays[1]); // local .codeignore/.continueignore
