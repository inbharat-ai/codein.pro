...
[**Quick Start**](#-quick-start) · [**Why CodeIn?**](#-codein-vs-paid-tools) · [**Features**](#-features) · [**Architecture**](#-system-architecture) · [**Multi-Agent Swarm**](#-multi-agent-swarm-engine) · [**Languages**](#-code-in-every-language-of-bharat) · [**Skills**](#-skills--automation) · [**Contributing**](#-contributing)
...
| Feature | Description |
| :--- | :--- |
| **GPU on Demand (RunPod)** | Full RunPod integration: browse GPUs with live pricing, spin up on-demand pods (A100, H100, RTX 4090), submit serverless inference jobs. 9 MCP tools for full pod and job lifecycle management. |
...
| **Onboarding Flow** | Guided first-run experience with mode selection, feature discovery, and AI-suggested prompts based on detected project type. |
...
| **Docker Sandbox** | Dropped capabilities, read-only root filesystem, resource limits, network isolation. |
...
Registered in the plugin system (`plugin-system.js`), these skills are available to all agents and can be invoked programmatically or through the swarm engine. Skills run inside the plugin sandbox with timeout protection (30s default), no network access, and no writes outside the workspace.
...
Setup via **Settings > Code Graph** or the agent API. GitNexus runs as a separate process (PolyForm Noncommercial license) — CodeIn communicates via the MCP protocol. All existing features work unchanged without it.
...
│   ├── src/main/                   #   App lifecycle + 10 backend services
