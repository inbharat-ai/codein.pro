...
const TYPE_COLORS: Record<string, string> = {
  ...
  agent_spawned: "text-codin-indigo-400",
  agent_terminated: "text-gray-400",
  task_submitted: "text-cyan-400",
  task_completed: "text-green-400",
  task_failed: "text-red-400",
  task_cancelled: "text-codin-saffron-400",
  ...
  permission_requested: "text-codin-saffron-400",
  ...
  if (events.length === 0) {
    return (
      <div>
        ...
        <p className="text-codin-fg-muted text-[10px]">No events yet</p>
      </div>
    );
  }
      ...
      <div className="scrollbar-thin max-h-48 space-y-0.5 overflow-y-auto pr-1">
        {[...events].reverse().map((ev) => (
          ...
          >
            ...
            <span
              className={`shrink-0 ${TYPE_COLORS[ev.type] || "text-codin-fg-secondary"}`}
            >
              {ev.type.replace(/_/g, " ")}
            </span>
            {ev.data && (
              <span className="text-codin-fg-muted truncate">
