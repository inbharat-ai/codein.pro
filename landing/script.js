...
function generateFallbackManifest() {
  return {
    ...
    assets: {
      win_x64: {
        name: "CodeIn-1.1.0-x64.exe",
        url: `https://github.com/${GITHUB_REPO}/releases/download/${RELEASE_TAG}/CodeIn-1.1.0-x64.exe`,
        sha256: "PENDING",
        size: "~120 MB",
      },
      win_x64_portable: {
        name: "CodeIn-1.1.0-x64-portable.exe",
        url: `https://github.com/${GITHUB_REPO}/releases/download/${RELEASE_TAG}/CodeIn-1.1.0-x64-portable.exe`,
        sha256: "PENDING",
        size: "~115 MB",
      },
      mac_arm64: {
        name: "CodeIn-1.1.0-arm64.dmg",
        url: `https://github.com/${GITHUB_REPO}/releases/download/${RELEASE_TAG}/CodeIn-1.1.0-arm64.dmg`,
        sha256: "PENDING",
        size: "~125 MB",
      },
      mac_x64: {
        name: "CodeIn-1.1.0-x64.dmg",
        url: `https://github.com/${GITHUB_REPO}/releases/download/${RELEASE_TAG}/CodeIn-1.1.0-x64.dmg`,
        sha256: "PENDING",
        size: "~130 MB",
      },
      linux_x64_appimage: {
        name: "CodeIn-1.1.0-x64.AppImage",
        url: `https://github.com/${GITHUB_REPO}/releases/download/${RELEASE_TAG}/CodeIn-1.1.0-x64.AppImage`,
        sha256: "PENDING",
        size: "~140 MB",
      },
      linux_x64_deb: {
        name: "CodeIn-1.1.0-amd64.deb",
        url: `https://github.com/${GITHUB_REPO}/releases/download/${RELEASE_TAG}/CodeIn-1.1.0-amd64.deb`,
        sha256: "PENDING",
        size: "~135 MB",
      },
    },
  ...
  if (!releaseExists) {
    container.innerHTML = `
      <div class="col-span-full text-center py-12 px-6 rounded-2xl border border-brand-500/20 bg-brand-500/5">
        ...
        <h3 class="text-xl font-bold text-white mb-2">${typeof t === "function" ? t("hero.download") + " CodeIn" : "Download CodeIn"}</h3>
        <p class="text-gray-400 mb-6 max-w-md mx-auto">CodeIn ${RELEASE_TAG} ${typeof t === "function" ? t("downloads.cta_desc") : "is available now. Get the current release from GitHub."}</p>
        <a href="${RELEASES_PAGE}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-brand-600 hover:bg-brand-500 text-white font-medium text-sm transition-all duration-200 shadow-lg shadow-brand-600/20">
          ...
          ${typeof t === "function" ? t("downloads.get_release") : "Get Current Release"}
        </a>
      </div>
    `;
    return;
  ...
  container.innerHTML = config.assets
    .map((asset) => {
      ...
      <div class="download-card relative p-6 rounded-2xl border ${borderColor} ${bgColor} hover:border-brand-500/40 transition-all duration-300 group">
        ${isPrimary ? `<div class="absolute -top-3 left-6"><span class="px-3 py-1 rounded-full bg-brand-500 text-white text-xs font-semibold shadow-lg shadow-brand-500/25">${typeof t === "function" ? t("downloads.recommended") : "Recommended"}</span></div>` : ""}
        ...
        <div class="mb-5">
          <div class="flex items-center gap-2 mb-1.5">
            ...
            <span class="text-xs text-gray-400">${typeof t === "function" ? t("downloads.sha256_label") : "SHA-256"}</span>
          </div>
        ...
        <a href="${data.url}" ${isDirectDownload ? `onclick="return handleDownloadClick(event, '${data.url}')"` : ""} class="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-xl ${isPrimary ? "bg-brand-600 hover:bg-brand-500 text-white shadow-lg shadow-brand-600/20" : "bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white border border-white/10"} font-medium text-sm transition-all duration-200">
          ...
          ${typeof t === "function" ? (isPrimary ? t("downloads.download_btn") : t("downloads.download_alt")) : isPrimary ? "Download" : "Download (alternate)"}
        </a>
      </div>
    `;
    })
...
function updateHeroForOS() {
  ...
  if (releaseExists) {
    if (btnText)
      btnText.textContent = `${typeof t === "function" ? t("hero.download") : "Download"} ${config?.label || "your OS"}`;
    if (osLabel)
      osLabel.textContent = `${typeof t === "function" ? t("hero.detected") : "Detected:"} ${config?.label || "Unknown OS"}`;
    if (ctaText)
      ctaText.textContent = `${typeof t === "function" ? t("hero.download") : "Download"} ${config?.label || "your OS"}`;
  } else {
    if (btnText)
      btnText.textContent =
        typeof t === "function"
          ? t("hero.download") + " CodeIn"
          : "Download CodeIn";
    if (osLabel) osLabel.textContent = `${RELEASE_TAG}`;
    if (ctaText)
      ctaText.textContent =
        typeof t === "function"
          ? t("hero.download") + " CodeIn"
...
async function copySha(hash) {
  try {
    ...
    showToast(
      typeof t === "function"
        ? t("downloads.sha256_label") + " copied"
...
function switchLang(code) {
  ...
  renderDownloadCards(activePlatform);
  updateHeroForOS();
  // Update label
