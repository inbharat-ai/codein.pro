...
runTest("Local packages are bundled", () => {
  ...
  // Check for code from @codein/config-yaml
  if (
    !bundleContent.includes("AssistantUnrolled") &&
    !bundleContent.includes("config-yaml")
  ) {
    throw new Error("@codein/config-yaml not properly bundled");
  }

  // Check for code from @codein/openai-adapters
  ...
  if (
    ...
    throw new Error("@codein/openai-adapters not properly bundled");
  }
});
