...
The Continue CLI uses esbuild to bundle the application along with local packages (`@codein/config-yaml` and `@codein/openai-adapters`) into a single distributable file. This ensures that users who install the CLI from npm don't need to worry about local file references.
...
- Include local packages (`@codein/config-yaml`, `@codein/openai-adapters`) directly in the bundle
...
2. Users install via `npm install -g @codein/cli`
