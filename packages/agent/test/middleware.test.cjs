...
const { RateLimiter, createRateLimiterMiddleware } = require('../src/middleware/rate-limiter');
const { createSecurityHeadersMiddleware } = require('../src/middleware/security-headers');
...
test('rate limiter resets per-minute bucket', () => {
  const limiter = new RateLimiter({ requestsPerMinute: 2, requestsPerHour: 100 });
  
  // Use up the minute bucket
  limiter.isAllowed('192.168.1.2');
  limiter.isAllowed('192.168.1.2');
  assert.strictEqual(limiter.isAllowed('192.168.1.2').allowed, false, 'Should be blocked');
...
test('rate limiter uses user-specific buckets', () => {
  ...
  assert.strictEqual(result.allowed, true, 'Different user should have separate limits');
...
test('security headers middleware sets correct headers', () => {
  ...
  assert.strictEqual(headers['X-Frame-Options'], 'DENY', 'Should set X-Frame-Options');
  assert.strictEqual(headers['X-Content-Type-Options'], 'nosniff', 'Should set X-Content-Type-Options');
  assert.strictEqual(headers['Content-Security-Policy'], "default-src 'self'", 'Should set CSP');
...
test('rate limiter middleware returns 429 when limited', () => {
  const limiter = new RateLimiter({ requestsPerMinute: 1, requestsPerHour: 100 });
  ...
  const req2 = {
    ...
    connection: { remoteAddress: 'ignored' },
    user: null,
  };
  ...
  assert.strictEqual(responseStatus, 429, 'Should return 429 Too Many Requests');
