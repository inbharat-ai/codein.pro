...
} from "../contract/EditContractApplier";
import {
...
function detectRunConfig(root: string) {
...
export class VsCodeMessenger {
  ...
  constructor(
    ...
    this.onWebview("contract/apply", async ({ data }) => {
      const backup = await applyEditContract(data.contract, this.ide, context);
      await context.globalState.update(
        "bharatcode.lastContractBackupId",
        backup.id,
      );
      this.webviewProtocol.send("contract/applied", { backupId: backup.id });
      return { backupId: backup.id };
    });

    this.onWebview("contract/rollback", async ({ data }) => {
      await rollbackEditContract(data.backupId, this.ide, context);
      this.webviewProtocol.send("contract/rolledBack", {
        backupId: data.backupId,
      });
    });

    const runTerminals = new Map<string, vscode.Terminal>();
    const runPermissionKey = "bharatcode.allowedRunCommands";

    const confirmCommand = async (commandText: string, detail: string) => {
      ...
      if (selection === "Always allow") {
        const allowed =
          context.workspaceState.get<string[]>(runPermissionKey) || [];
        await context.workspaceState.update(runPermissionKey, [
          ...new Set([...allowed, commandText]),
        ]);
        return true;
      }
    ...
    this.onWebview("run/detect", async () => {
      const root = getWorkspaceRoot();
      if (!root) {
        throw new Error("No workspace folder found");
      }
      return detectRunConfig(root);
    });

    this.onWebview("run/execute", async ({ data }) => {
      const root = data.cwd || getWorkspaceRoot();
      if (!root) {
        throw new Error("No workspace folder found");
      }

      const commandParts = [data.command, ...(data.args || [])].filter(Boolean);
      ...
      const allowed =
        (context.workspaceState.get<string[]>(runPermissionKey) || []).includes(
          commandText,
        ) ||
        (await confirmCommand(
          commandText,
          `Allow BharatCode to run: ${commandText}?`,
        ));
      ...
      runTerminals.set(runId, terminal);

      return {
        runId,
        previewUrl: data.port ? `http://localhost:${data.port}` : undefined,
      };
    });

    this.onWebview("run/stop", async ({ data }) => {
      const terminal = runTerminals.get(data.runId);
      if (terminal) {
        terminal.dispose();
        runTerminals.delete(data.runId);
      }
    });

    this.onWebview("run/openPreview", async ({ data }) => {
      await vscode.env.openExternal(vscode.Uri.parse(data.url));
    });

    this.onWebview("git/status", async () => {
      const root = getWorkspaceRoot();
      if (!root) {
        return { branch: null, changes: [] };
      }

      try {
        const gitRoot = await execGit("git rev-parse --show-toplevel", root);
        const branch = await execGit(
          "git rev-parse --abbrev-ref HEAD",
          gitRoot,
        );
        const raw = await execGit("git status --porcelain", gitRoot);
    ...
    this.onWebview("git/commit", async ({ data }) => {
      const root = getWorkspaceRoot();
      if (!root) {
        throw new Error("No workspace folder found");
      }
      ...
      const confirmed = await confirmCommand(
        "git commit",
        "Allow BharatCode to stage and commit changes?",
      );
      if (!confirmed) {
      ...
      const gitRoot = await execGit("git rev-parse --show-toplevel", root);
      if (data.addAll !== false) {
        await execGit("git add -A", gitRoot);
      }
      const commit = await execGit(
        `git commit -m ${sanitizeShellArgument(message)}`,
        gitRoot,
      );
      return { commitHash: commit.split("\n")[0] };
    });

    this.onWebview("git/push", async ({ data }) => {
      const root = getWorkspaceRoot();
      if (!root) {
        throw new Error("No workspace folder found");
      }
      ...
      const confirmed = await confirmCommand(
        "git push",
        "Allow BharatCode to push commits?",
      );
      if (!confirmed) {
      ...
      const branch =
        data.branch ||
        (await execGit("git rev-parse --abbrev-ref HEAD", gitRoot));
      const remote = data.remote || "origin";
      const output = await execGit(
        `git push ${sanitizeShellArgument(remote)} ${sanitizeShellArgument(branch)}`,
        gitRoot,
      );
      return { ok: true, output };
    });

    this.onWebview("git/checkout", async ({ data }) => {
      const root = getWorkspaceRoot();
      if (!root) {
        throw new Error("No workspace folder found");
      }
      ...
      const confirmed = await confirmCommand(
        "git checkout",
        `Allow BharatCode to switch to ${data.branch}?`,
      );
      if (!confirmed) {
      ...
      const gitRoot = await execGit("git rev-parse --show-toplevel", root);
      const flag = data.create ? "-b " : "";
      await execGit(
        `git checkout ${flag}${sanitizeShellArgument(data.branch)}`,
        gitRoot,
      );
      return { branch: data.branch };
    });

    this.onWebview("deploy/generate", async ({ data }) => {
      const root = getWorkspaceRoot();
      if (!root) {
        throw new Error("No workspace folder found");
      }

      const runConfig = detectRunConfig(root);
      const buildCommand =
        runConfig.packageManager === "npm"
          ? "npm run build"
          : runConfig.packageManager
            ? `${runConfig.packageManager} build`
            : "npm run build";

      const outputDir =
        runConfig.projectType === "vite"
          ? "dist"
          : runConfig.projectType === "cra"
            ? "build"
            : runConfig.projectType === "next"
              ? ".next"
              : "dist";
      ...
      let instructions = "";
      let files = [] as Array<{ path: string; created: boolean }>;
      ...
      if (data.target === "vercel") {
        ...
        instructions =
          "Run `vercel` from the project root, then select the created settings.";
      }

      if (data.target === "netlify") {
        const netlifyToml = `[build]\ncommand = \"${buildCommand}\"\npublish = \"${outputDir}\"\n`;
        files.push(
          await createFile(path.join(root, "netlify.toml"), netlifyToml),
        );
        instructions =
          "Run `netlify deploy` from the project root and follow the prompts.";
      }
      ...
      if (data.target === "firebase") {
        ...
        instructions =
          "Run `firebase init hosting` then `firebase deploy` from the project root.";
      }

      return { files, instructions };
    });

    this.onWebview("mcp/list", async () => {
